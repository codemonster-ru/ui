...
- [x] `CMUI-197` Restore the complete showcase information architecture and representative example
...
- [x] `CMUI-G012` The restored showcase and all migrated visual cases pass browser screenshot
...
| 2026-08-27 | Attribute the open Core screenshot differences to a stale baseline rather than component regressions.                           | Capturing the baseline commit `2b87f52d` and `HEAD` produces bit-identical images in both suites, and no shared package or showcase source changed between them. The differences date from `b0555a3b`, which added page-height stabilisation to the capture script without a matching baseline refresh.                                                                                                                                                                                                                                                                  | `CMUI-195`, `CMUI-196`, `CMUI-G012`            |
| 2026-08-27 | Move macOS visual CI from `macos-15` to `macos-26`.                                                                             | The showcase renders in the macOS system font, so baselines are tied to the runner's OS release; matching CI to the maintainer workstation restores local verification, and `macos-latest` already points at `macos-26`.                                                                                                                                                                                                                                                                                                                                                 | `CMUI-193`, `CMUI-G012`                        |
| 2026-08-27 | Defer bundling a self-hosted `Inter` webfont until after M12.                                                                   | Neither the frozen VueForge reference nor CodeMonster UI ships the font its tokens declare, so both render in `.SF NS`; bundling it would remove the OS dependency but change the frozen visual language that M12 must preserve.                                                                                                                                                                                                                                                                                                                                         | Post-M12 backlog                               |
| 2026-08-28 | Supersede the 2026-08-27 stale-baseline attribution; the open Core differences are real.                                        | Capturing the reference commit and `HEAD` with one script on one machine reports 62 changed Core frames and none in Colors, Layouts, or Icons, matching the 62 recorded on 2026-08-16. The committed baseline was captured from the working tree, so the gate compared the product against itself and could not observe them.                                                                                                                                                                                                                                            | `CMUI-195`, `CMUI-196`, `CMUI-G012`            |
| 2026-08-28 | Capture the showcase and state baselines from a reference-commit worktree.                                                      | Regenerating a baseline from the working tree resets the gate to zero without changing the product, which is how the 2026-08-18 refresh cleared the recorded differences; the capture script now probes readiness in a way both the reference and the current application shell satisfy.                                                                                                                                                                                                                                                                                 | `CMUI-192`, `CMUI-193`                         |
| 2026-08-28 | Keep the cross-platform baseline outside this repair.                                                                           | It still stores the current Vue render and checks Razor against it, so cross-platform parity remains provable only against the reviewed reference under its own item.                                                                                                                                                                                                                                                                                                                                                                                                    | `CMUI-199`                                     |
| 2026-08-28 | Reopen `CMUI-180` and rebuild Select and DatePicker as component-owned controls.                                                | The reference renders both as authored markup, and a native control cannot reach it: the browser forces its own line height on `select`, so the small control measures one pixel taller than the reference no matter what the stylesheet asks for.                                                                                                                                                                                                                                                                                                                       | `CMUI-180`, `CMUI-195`, `CMUI-G012`            |
| 2026-08-28 | Sequence the remaining visual work before that rebuild.                                                                         | The mechanical porting defects are cheap to close and their removal shows how much of the residual actually belongs to the two controls; Select also precedes DatePicker because the reference composes one inside the other.                                                                                                                                                                                                                                                                                                                                            | `CMUI-195`, `CMUI-196`                         |
| 2026-08-29 | Render the same authored markup on both platforms and let scripting supply interaction.                                         | Annabel Razor must reproduce the Vue rendering rather than fall back to a browser-drawn control, so the server emits the component-owned markup and the shared runtime wires up interaction after load; a form still submits the server-rendered value through the hidden input when scripting is unavailable, but the value cannot be changed without it.                                                                                                                                                                                                               | `CMUI-180`, `CMUI-195`                         |
| 2026-08-29 | Hand required-field validation for Select to the application.                                                                   | A component-owned control cannot reach the browser's constraint validation: a hidden input is excluded from it and the trigger is a button. Submission still works through the hidden input, so only the refusal to submit an empty required field is lost, and marking it `aria-required` leaves the state announced.                                                                                                                                                                                                                                                   | `CMUI-180`, `CMUI-195`                         |
| 2026-08-29 | Build the collapsible GroupBox recipe on a native `legend` instead of a reconstructed rule.                                     | A fieldset draws its border from the legend's vertical centre, so hiding the legend and redrawing the top rule with a pseudo-element left the side borders and both upper corners eighteen pixels high; placing the disclosure button inside the legend reproduces the reference geometry exactly.                                                                                                                                                                                                                                                                       | `CMUI-195`                                     |
| 2026-08-29 | Track Core parity by comparing the reference commit and `HEAD` in one browser rather than against the committed baseline.       | Both sides then render under identical conditions, so the count measures the product instead of the workstation: the 62 changed Core frames are now 18, holding 363 differing pixels, and Colors, Layouts, Icons, CodeBlock, and Playground are identical. Every remaining cluster is at most twenty pixels of antialiasing along a curve.                                                                                                                                                                                                                               | `CMUI-195`, `CMUI-196`                         |
| 2026-08-29 | Supersede the same-day count of 18 frames and 363 pixels; the clean figure is 16 frames and 237 pixels.                         | That count came from a dev server carried through a long editing session, which serves module state the working tree no longer matches. Restarted servers reproduce 237 across repeated runs, and the frozen reference capture is byte-identical across three sessions.                                                                                                                                                                                                                                                                                                  | `CMUI-195`, `CMUI-196`                         |
| 2026-08-29 | Stop closing the residual Core pixels and put the gate threshold to the maintainer.                                             | The remainder is not authored geometry: adding `will-change: transform`, which changes no layout, colour, or position, takes the spinner residue to zero and halves the clear-icon residue, so the two documents rasterise identical subtrees through different compositing paths. Icon markup, package version, resolved size, colour, stroke, the icon-button wrapper, reduced-motion handling, and the mask's containing block were each measured and matched or ruled out. Closing it would mean adding compositing hints to shipped components to mask an artefact. | `CMUI-195`, `CMUI-G012`                        |
| 2026-08-29 | Supersede that decision: the residual pixels were authored, not rasterisation, and Core now matches the reference exactly.      | Chrome's layer tree named the cause the computed styles hid. Six scroll viewports that the reference positions were left static, which gave the page an extra squashing layer and rasterised everything below it differently; the table loading mask was anchored to a rounded wrapper instead of the plain viewport; and the select chevron was drawn with a mask where the reference strokes a polyline. With those closed, all 312 frames report zero differing pixels, so the gate needs no threshold change.                                                        | `CMUI-195`, `CMUI-196`, `CMUI-G012`            |
| 2026-08-29 | Capture the cross-platform baseline from the reference package instead of the adapters, closing `CMUI-199`.                     | The reviewed baseline was only ever regenerable from the product, so it recorded the adapters: its select frame shows a chevron beside the clear action, which the reference component does not render for that case. A fixture mounting the reference package from the refresh's own worktree makes nine of the thirteen cases match the adapters exactly.                                                                                                                                                                                                              | `CMUI-199`, `CMUI-G012`                        |
| 2026-08-29 | Treat the four cases the honest baseline fails as open component work rather than baseline noise.                               | The reference draws its adornments as icons where the shipped Select, Input, and DatePicker substitute text characters, the shipped DatePicker is still a native `input[type=date]` against a component-owned reference, and the reference has no multi-item Accordion at all, so that case cannot be captured from it.                                                                                                                                                                                                                                                  | `CMUI-180`, `CMUI-199`                         |
| 2026-08-29 | Give the reference fixture the frame the reference application establishes, not the one our foundation does.                    | Stating more than that application states drifts the glyphs by a subpixel: an over-specified frame left the enhanced input twenty-one pixels apart with every box, style, and path already matching. Reproducing the reference playground's own reset closed it to zero.                                                                                                                                                                                                                                                                                                 | `CMUI-199`                                     |
| 2026-08-29 | Draw component-owned adornments as icons in Select and Input on both adapters.                                                  | The reference strokes an xmark and an eye where the shipped controls printed a multiplication sign and a circle character, and the adornment slot was pinned to a square that clipped textual adornments. Eleven of the thirteen cross-platform cases now match the reference exactly, on both adapters, and the showcase stays at zero.                                                                                                                                                                                                                                 | `CMUI-180`, `CMUI-199`                         |
| 2026-08-29 | Pin the capture locale, closing a baseline that depended on the machine that took it.                                           | The reference DatePicker formats through the browser's locale, so the same commit rendered `13.08.26` on a Russian workstation and `08/13/26` on CI. Both capture scripts now override the locale, and the shipped control formats through the same fixed one so a server render and the client that takes it over agree.                                                                                                                                                                                                                                                | `CMUI-199`, `CMUI-G012`                        |
| 2026-08-29 | Complete `CMUI-180` by rebuilding DatePicker as a component-owned control on both adapters.                                     | The shipped picker was still a native `input[type=date]`, which no stylesheet can reconcile with the reference's authored trigger. Twelve of the thirteen cross-platform cases now match the reference exactly on both adapters, and the showcase stays at zero. Its calendar fills its dated content on open, because a grid rendered from the current date would make the canonical case expire.                                                                                                                                                                       | `CMUI-180`, `CMUI-199`                         |
...
| 2026-09-01 | Demonstrate components with no VueForge ancestor in a separate example rather than in the frozen showcase.                      | The frozen showcase is pinned to `fd793696` and its shell renders a section navigation on every route, so any new section or demo shifts page height and changes frames the gate compares. Adding nine post-migration components to it reported 112 changed and 4 unexpected frames while nothing about the migrated components had regressed. `examples/ui-showcase` holds components that have no predecessor to match, which keeps the frozen example a live regression guard instead of forcing a rebaseline that would compare the product against itself.          | `CMUI-G012`, `CMUI-197`                        |
