...
test('preserves the frozen Accordion surface and trigger geometry', () => {
  assert.match(
    css,
    /\.cm-accordion\s*\{[^}]*border: var\(--cm-border-width\) solid var\(--cm-color-border-default\);/s,
  );
  ...
  assert.match(
    css,
    /\.cm-accordion__item:first-child \.cm-accordion__trigger[^}]*border-start-start-radius: var\(--cm-radius-surface\);/s,
  );
  assert.match(
    css,
    /\.cm-accordion__item:last-child \.cm-accordion__trigger[^}]*border-end-end-radius: var\(--cm-radius-surface\);/s,
  );
});
...
test('maps frozen expanded, focus, icon, and panel visuals onto canonical regions', () => {
  assert.match(
    css,
    /\.cm-accordion__trigger\[aria-expanded='true'\][^}]*color: var\(--cm-color-selected-foreground\);/s,
  );
  ...
  assert.match(
    css,
    /\.cm-accordion__panel\s*\{[^}]*border-block-start: var\(--cm-border-width\) solid var\(--cm-color-border-divider\);/s,
  );
  assert.match(css, /\.cm-accordion__panel\s*\{[^}]*font-size: var\(--cm-font-size-xl\);/s);
  assert.match(
    css,
    /\.cm-accordion__item:last-child \.cm-accordion__panel[^}]*border-end-end-radius: var\(--cm-radius-surface\);/s,
  );
});
